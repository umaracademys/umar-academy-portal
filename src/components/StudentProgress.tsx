            ...
            <div className="space-y-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                  <button className="px-4 py-2 min-h-[44px] bg-primary text-white rounded-lg hover:bg-primary/90 transition body-text font-medium">
                    Mark present
                  </button>
                  <button className="px-4 py-2 min-h-[44px] border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition body-text font-medium">
                    Export
                  </button>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                <div className="px-4 py-3 border-b border-gray-200">
                  ...
                  <p className="caption mt-0.5 text-gray-600">Class dates and status</p>
                </div>
                <div className="divide-y divide-gray-200">
                  {progressData.attendance.map((record, index) => (
                    <div
                      key={index}
                      className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-4 min-h-[44px]"
                    >
                      <div className="min-w-0">
                        <p className="body-text font-medium text-gray-900">{record.class}</p>
                        <p className="caption text-gray-600">{new Date(record.date).toLocaleDateString()}</p>
                      </div>
                      <span
                        className={`inline-flex shrink-0 px-2.5 py-1 text-sm font-medium rounded-full border ${
                          record.status === 'present'
                            ? 'bg-gray-100 text-gray-800 border-gray-200'
                            : 'bg-gray-100 text-gray-600 border-gray-200'
                        }`}
                      >
                        {record.status === 'present' ? 'Present' : 'Absent today'}
                      </span>
                    </div>
                  ))}
                </div>
