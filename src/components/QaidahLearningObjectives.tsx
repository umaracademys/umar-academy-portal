...
interface QaidahLearningObjectivesProps {
  ...
  page: number;
  onClose?: () => void;
  annotations?: Array<{ type: string; note?: string; text?: string }>; // Phase 3: For auto-generating objectives
  onMistakeMark?: (mistakeType: string) => void; // Phase 3: One-tap mistake marking
}
...
const QaidahLearningObjectives: React.FC<QaidahLearningObjectivesProps> = ({
  ...
  // Phase 3: Auto-generate learning objectives from selected letters/rules
  useEffect(() => {
    if (editData.letters.length > 0 || editData.rules.length > 0) {
      const lettersText = editData.letters.length > 0 
        ? `Letters: ${editData.letters.join(', ')}` 
        : '';
      const rulesText = editData.rules.length > 0 
        ? `Rules: ${editData.rules.join(', ')}` 
        : '';
      
      if (lettersText || rulesText) {
        const autoGenerated = [lettersText, rulesText].filter(Boolean).join('. ');
        if (!editData.learningObjectives || editData.learningObjectives === '') {
          setEditData(prev => ({ ...prev, learningObjectives: autoGenerated }));
        }
      }
    }
  }, [editData.letters, editData.rules]);
  ...
  // Phase 3: Common mistakes for one-tap marking
  const commonMistakes = [
    'Madd', 'Holding', 'Memory', 'Ikhfa', 'Tech', 'Letter', 
    'Heavy Letter', 'No Rounding Lips', 'Heavy H', 'Light L', 'Atkee'
  ];
  ...
  return (
    <div className="fixed top-20 right-4 z-50 w-80 max-h-[calc(100vh-6rem)] bg-white rounded-lg shadow-2xl border border-primary-300 flex flex-col overflow-hidden">
      ...
      {/* Phase 5: History Panel - Enhanced with smooth transitions */}
      {showHistory && (
        <div className="px-2.5 py-2 bg-gray-50 border-b border-gray-200 max-h-60 overflow-y-auto transition-all duration-300">
            ...
            <div className="space-y-1.5">
              {history.map((item, idx) => (
                <button
                  ...
                  className="w-full text-left px-2 py-2 text-xs bg-white border border-gray-300 rounded hover:bg-primary-50 hover:border-primary-300 transition-all duration-200 transform hover:scale-[1.02]"
                >
                  <div className="font-medium text-gray-800">{new Date(item.teachingDate || '').toLocaleDateString()}</div>
                  <div className="text-gray-600 mt-0.5">
                    {item.letters.length} letters, {item.rules.length} rules
                  </div>
                  {item.learningObjectives && (
                    <div className="text-gray-500 mt-1 truncate">
                      {item.learningObjectives.substring(0, 50)}...
                    </div>
                  )}
                </button>
              ))}
      ...
      <div className="flex-1 overflow-y-auto p-2.5 space-y-2.5">
        ...
        {/* Phase 3: One-Tap Mistake Marking */}
        {onMistakeMark && (
          <div>
            <h4 className="font-semibold text-primary-700 mb-1.5 text-xs flex items-center gap-1.5">
              <span className="text-sm">❌</span>
            ...
            <div className="flex flex-wrap gap-1">
              {commonMistakes.map((mistake) => (
                <button
                  key={mistake}
                  onClick={() => onMistakeMark(mistake)}
                  className="px-2 py-1 bg-red-100 hover:bg-red-200 text-red-800 border border-red-300 rounded text-xs font-medium transition-colors active:scale-95"
                  title={`Mark ${mistake} mistake`}
                >
                  ❌ {mistake}
                </button>
              ))}
